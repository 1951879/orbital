...
export const Manta: React.FC<{ playerId?: number, throttle?: number }> = ({ playerId = 1, throttle }) => {
  ...
  return (
     <group scale={[0.3, 0.3, 0.3]}>
        ...
        <group position={[0.8, 0.2, -1.5]}>
            <JetExhaust color={colors.highlight} scale={1.0} length={2.0} throttle={effectiveThrottle} />
        </group>
        <group position={[-0.8, 0.2, -1.5]}>
            <JetExhaust color={colors.highlight} scale={1.0} length={2.0} throttle={effectiveThrottle} />
        </group>
