...
export const Tempest: React.FC<{ playerId?: number, throttle?: number }> = ({ playerId = 1, throttle }) => {
  ...
  return (
     <group scale={[0.38, 0.38, 0.38]}>
        ...
        <group position={[1.2, 0, -2.3]}>
            <JetExhaust color={colors.highlight} scale={0.8} length={2.5} throttle={effectiveThrottle} />
        </group>
        <group position={[-1.2, 0, -2.3]}>
            <JetExhaust color={colors.highlight} scale={0.8} length={2.5} throttle={effectiveThrottle} />
        </group>
