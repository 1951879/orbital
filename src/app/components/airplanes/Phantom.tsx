...
export const Phantom: React.FC<{ playerId?: number, throttle?: number }> = ({ playerId = 1, throttle }) => {
  ...
  return (
     <group scale={[0.4, 0.4, 0.4]}>
        ...
        <group position={[0.5, 0.1, -1.0]}>
            <JetExhaust color={colors.highlight} scale={0.7} length={2.0} throttle={effectiveThrottle} />
        </group>
        <group position={[-0.5, 0.1, -1.0]}>
            <JetExhaust color={colors.highlight} scale={0.7} length={2.0} throttle={effectiveThrottle} />
        </group>
